...
class SparseOccTRT(SparseOcc):
    ...
    def extract_feat(self, img, img_shape, ori_shape, pad_shape):
        ...
        if self.data_aug is not None:
            ...
            for b in range(B):
                ...
                img_shape[b] = torch.tensor([img_shape for _ in range(N)], dtype=torch.float32, device=img.device)
                ori_shape[b] = torch.tensor([img_shape for _ in range(N)], dtype=torch.float32, device=img.device)
    ...
    def forward_trt(self, img, img_filenames, img_shape, ori_shape, pad_shape, lidar2img, ego2lidar, img_timestamp):
    ...
    def simple_test(self, img, img_filenames, img_shape, ori_shape, pad_shape, lidar2img, ego2lidar, img_timestamp):
        ...
        img_shape = (H, W, C)
        img_shape[0] = torch.tensor([img_shape for _ in range(len(img_filenames))], device=img.device)
        ori_shape[0] = torch.tensor([img_shape for _ in range(len(img_filenames))], device=img.device)
        pad_shape[0] = torch.tensor([img_shape for _ in range(len(img_filenames))], device=img.device)
        ...
        for i in range(num_frames):
            ...
            img_shape_curr = torch.tensor([img_shape[0][i] for i in img_indices], device=img.device)
            ori_shape_curr = torch.tensor([ori_shape[0][i] for i in img_indices], device=img.device)
            pad_shape_curr = torch.tensor([pad_shape[0][i] for i in img_indices], device=img.device)
            lidar2img_curr = torch.stack([lidar2img[0][i] for i in img_indices], dim=0)
            ego2lidar_curr = torch.stack([ego2lidar[0][i] for i in img_indices], dim=0) 
            ...
            if img_filenames[img_indices[0]] in self.memory:
                ...
                img_feats_curr = self.extract_feat(img[:, i], img_shape_curr, ori_shape_curr, pad_shape_curr)
        ...
        for i in range(1, len(img_metas_list)):
            ...
            lidar2img = torch.cat([ori_shape, img_metas_list[i][2]], dim=0)
            ego2lidar = torch.cat([pad_shape, img_metas_list[i][3]], dim=0)
