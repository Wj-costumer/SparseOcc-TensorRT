...
def pad_multiple(inputs, img_shape, pad_shape, size_divisor=32):
    ...
    B, N = img_shape.shape[:2]

    for b in range(B):
        img_shape[b] = torch.tensor([(img_h + pad_h, img_w + pad_w, 3) for _ in range(N)], device=inputs.device)
        pad_shape[b] = torch.tensor([(img_h + pad_h, img_w + pad_w, 3) for _ in range(N)], device=inputs.device)

    if pad_h == 0 and pad_w == 0:
        return inputs, img_shape, pad_shape
    else:
        return F.pad(inputs, [0, pad_w, 0, pad_h], value=0), img_shape, pad_shape
